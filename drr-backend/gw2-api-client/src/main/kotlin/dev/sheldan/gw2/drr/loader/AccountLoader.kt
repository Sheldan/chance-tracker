package dev.sheldan.gw2.drr.loader

import dev.sheldan.gw2.drr.models.*
...
@RequestScope
class AccountLoader(
    @Autowired var charClient: GWCharactersClient,
    @Autowired var accountClient: GWAccountClient,
    @Autowired var miscellaneousClient: GWMiscellaneousClient,
    @Autowired var itemClient: GWItemsClient
) {
    ...
    fun getWallet(): AccountWalletView {
        ...
        val enrichedCurrencies = wallet.mapNotNull { walletCurrency ->
            ...
            currency?.let {
                Currency(walletCurrency.id, walletCurrency.value, it.name, it.description, it.icon, it.order)
            }
        }
    ...
    fun getBank(): AccountVaultView {
        ...
        val itemStats = itemIds.chunked(200).flatMap { (itemClient.getItems(it)) }.associateBy { it.id }
        items.forEach { item ->
    ...
    fun getMaterials(): AccountMaterialView{
        ...
        val actualMaterials = itemIds.chunked(200).flatMap { (itemClient.getItems(it)) }.associateBy { it.id }
        items.forEach { item ->
